...
- **Unified Textured Inventory Interface (<kbd>E</kbd> Key)**: Texture-skinned container interface (`src/menu/creative_inventory.rs`) based on a unified 190×152 px container panel (`assets/textures/interfaces/containers/inventory.png` and `inventory_creative.png`) rendered at 3× integer scale (570×456 px) using nearest-neighbor sampling. Centered horizontally and vertically:
  ...
  - **Personal Inventory Panel**: Displays the dedicated `"Personal Inventory"` title ($X=24, Y=32$), two action button placeholders ($X=138, 156$), an 8×4 storage grid (32 slots) mapped to `PlayerInventory`, and a 1×8 hotbar mirror row at $Y=128$.
  - **Creative Inventory Panel**: Displays the dedicated `"Creative Inventory"` title, an interactive search bar ($X=99..167, Y=33..42$), an 8×4 palette grid of all registered blocks with live filtering, a pixel-art scrollbar ($X=175..186$) with draggable `inventory_scroller.png` thumb, and the 1×8 hotbar mirror row.
  - **Search Bar Ergonomics & Text Selection**:
    ...
    - **Caps Lock & Shift Uppercase**: Full support for Caps Lock and Shift modifiers for uppercase typing.
...
- **Minecraft/Xaero-Style Minimap HUD**: Top-right square HUD with parchment `map_background.png` frame extending outward around the 192×192 dynamic canvas, bright white cardinal indicators (<kbd>N</kbd>, <kbd>S</kbd>, <kbd>E</kbd>, <kbd>W</kbd>) inset over terrain, real-time readout in logical sequence (`Coordinates: XYZ: ...`, `Date: Day ..., HH:MM, season`, `Biome: ...`), and live directional player `marker_red.png` rotating with camera yaw via GPU `UiTransform`. Powered by an asynchronous 2D cache (`MapCache`) with North-up topographic hill shading and precomputed surface colors for zero-noise 250+ FPS updates.
...
- **Custom Font & Universal Drop Shadows**: Centralized typography via `AppFont` and `FontPlugin` pointing to `assets/fonts/CutePixel.ttf`, universally applied across all in-game text elements (inventory titles, hotbar slot numbers, target HUD, minimap footer, settings/pause menus, and F3 dev stats) with crisp drop shadows (`TextShadow`) for clear contrast on all backgrounds.
...
- **Debug Overlays**: Minimal HUD by default displaying performance & geometry metrics (FPS, 1% low FPS, Min/Max FPS, loaded chunk counts, active mesh vertices and triangles), Extended Technical Debug HUD on <kbd>F3</kbd> (adding dynamic camera `Direction`, player XYZ/chunk coordinates, power source, dynamic throttle status, and biome climate diagnostics), HUD toggle on <kbd>Shift+F3</kbd>, and chunk debug borders on <kbd>F2</kbd>.
